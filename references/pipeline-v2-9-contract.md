...
1. **不再以 launcher 脚本 作为默认入口**：把 v2.7 / v2.8 的经验内化到规则中，恢复为 **main 在主会话中直接逐步 spawn 各 agent**。
2. **新增 Lite 快车道**：对于适合“先产品打磨 / 先轻量规划 / 再决定是否深入”的任务，可插入 `founder-office-hours` + `autoplan-lite`，而不是所有任务都走完整重链。
3. **新增官方插件层**：`founder-office-hours`、`autoplan-lite`、`review-gate`、`qa-browser-check`、`release-retro` 被正式吸收进星链，而不是另起一套平行流程。
4. **质量门前移并收硬**：`review-gate` 成为 coding 后的强制门，不允许 coding announce 直接等价于完成。
...
└── 步骤 1：任务分级 + 类型分析 + 路由选择
    ...
    │   ├── 步骤 2：coding
    │   ├── 步骤 3：review-gate（强制）
    │   ├── 步骤 4：test / qa-browser-check（按任务类型）
    │   ├── 步骤 5：docs（按需）
    │   ├── 步骤 6：main 汇总交付
    │   ├── 步骤 7：main 可靠通知
